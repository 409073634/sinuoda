...
<script>
  ...
  import { SORT_PROPS } from '@/app/basic/supplier/supplier.config'

  export default {
    name: 'SupplierSortDialog',
    ...
    computed: {
      ...
      inputOptions () {
        let rootNode = {
          id: '0',
          text: '根目录',
          children: getTreeData(JSON.parse(JSON.stringify(this.tree || [])), 'supplierSortDtoList', {text: 'name'})
        }
        return {
          pid: {
            data: [ rootNode ]
    ...
    methods: {
      saveSort () {
        ...
        if (this.sort.id) {
          api.supplierSort.update(this.sort).then(onSuccess)
        } else {
          api.supplierSort.add(this.sort).then(onSuccess)
        }
      },
      cancelSort () {
